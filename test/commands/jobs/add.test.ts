...
describe('jobs:add', () => {
  test
  .stub(cli, 'action.start', () => async () => 'Adding Heroku Scheduler job')
  .nock('https://api.heroku.com', api => api
    .get('/apps/example/config-vars')
    .reply(200, {SCHEDULERCTL_API_TOKEN: 'api-token'})
  )
  .nock('https://api.schedulerctl.com', api => api
    .post('/jobs', body => body.command && body.frequency && body.dynoSize)
    .reply(200, {message: 'Heroku Scheduler job created.', job: { id: '1', command: 'pwd', frequency: '*/10 * * * *', dynoSize: 'Free'}})
  )
  .stdout()
  .command([
    'jobs:add',
